...
$(function(){
	<%--增加查询条件--%>
	$.ajax({
		url: "${path}/servlet/switchport/SwitchportAction?func=PortSettingPrf2",
		data: {switchId: "${switchId}", level: 3, portId: "${portInfo.port_id}" },
		type: "post",
		dataType: "html",
						...
						<c:if test="${empty prfData.tbody.data}">
							<script>
								$("#switchportInfopageNub").getLinkStr({
									...
									isShowJump: true,
									ajaxRequestPath: "${path}/servlet/switchport/SwitchportAction?func=PortPrfPage&portId=${portInfo.port_id}&level=3&tablePage=1&switchId=${switchId}",divId:'dataContent'
								});
								$("#exportCSV").unbind();
		...
		<div class="box span12">
			...
			<div class="box-content"  style="overflow:auto;width:98%;min-height:180px;" id="dAlertContent">
				...
				<c:if test="${not empty deviceLogPage.data}">
					<script>
					$("#alertListNub").getLinkStr({pagecount:"${deviceLogPage.totalPages}",curpage:"${deviceLogPage.currentPage}",numPerPage:"${deviceLogPage.numPerPage}",isShowJump:true,ajaxRequestPath:"${path}/servlet/alert/DeviceAlertAction?func=AjaxPage&topId=${switchId}&resourceId=${portInfo.port_id}&resourceType=Switch",divId:'dAlertContent'});
					</script>
				</c:if>
			</div>
		</div>
