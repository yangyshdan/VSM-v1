...
var rankId = "${rankInfo.storage_extent_id}";
$(function(){
	<%--增加查询条件--%>
	$.ajax({
		url: "${path}/servlet/rank/RankAction?func=RankSettingPrf2",
		data: {subSystemID: "${subSystemID}", level: 3, rankId: "${rankInfo.storage_extent_id}" },
		type: "post",
		dataType: "html",
		success: function(result){
			$("#queryPage").html(result);
			$("#devtypeAndDevice").hide();
			var $opts = $("select#prfField option");
			if($opts.length == 0){
				$("ul#myTab a[href='#prfTab']").parent().hide();
			}
			else {
				doListRefresh();
			}
...
function doListRefresh(){
	loadData("${path}/servlet/rank/RankAction?func=RankPrfPage",{level:3,subSystemID:subSystemID,rankId:rankId},$("#perfChart"));
}
</script>
<style>
.spetable td{
...
<ul class="dashboard-list" style="margin-bottom: 10px;">
		<li style="padding-top: 0px; padding-bottom: 20px;">
			<a href="#">
				<img class="dashboard-avatar" style="border-width: 0px;" src="${path}/resource/img/project/rank.png" alt="StorageSystem">
			</a>
			<span style="font-size:25px;">${rankInfo.the_display_name}</span>
			<br>
			<strong>Status:</strong>
			<span>${rankInfo.the_operational_status}</span>
		</li>
</ul>
<div id="content">
...
<div class="tab-pane active" id="detailTab">
	<div class="box-content" style="width: 98%;  padding-top:10px;">
		<table class="table configTable" style="margin-bottom:0px;width:49%;float:left;">  
			  <tbody>
				<tr>
					<th><h4>存储系统</h4></th>
					<td class="center">${rankInfo.sub_name}</td>
				</tr>
				<tr>
					<th><h4>总容量(G)</h4></th>
					<td class="center"><fmt:formatNumber value="${rankInfo.the_total_space}" pattern="0.00" /></td>
				</tr>
				<tr>
					<th><h4>已用容量(G)</h4></th>
					<td class="center"><fmt:formatNumber value="${rankInfo.the_used_space}" pattern="0.00" /></td>
				</tr>
			  </tbody>
		 </table>  
		  <table class="table configTable" style="margin-bottom:0px;width:49%;float:left;">
			  <tbody>
				<tr>
					<th><h4>扩展卷数</h4></th>
					<td class="center">
						${rankInfo.the_extent_volume}
					</td>                                       
				</tr>
				<tr>
					<th><h4>空闲容量(G)</h4></th>
					<td class="center"><fmt:formatNumber value="${rankInfo.the_available_space}" pattern="0.00" /></td>
				</tr>
				<tr>
					<th><h4>存储池</h4></th>
					<td class="center">
	...
	<div class="row-fluid">
		<div class="box span12">
			<div class="box-header well">
				...
				<div class="box-icon">
					<!--<a href="javascript:void(0)" class="btn btn-setting btn-round" title="设置" onclick="Rank.settingPrf('${subSystemID}',3,'${rankInfo.storage_extent_id}')" data-rel='tooltip'><i class="icon-cog"></i></a>-->
					...
					<a href="javascript:void(0);" class="btn btn-round" title="导出" id="exportPrfCSV"><i class="icon-download-alt" data-rel='tooltip'></i></a>
				</div>
			</div>
			...
			<div class="box-content" style="min-height:300px;">
				...
				<div id="perfChart" class="tab-content" style="overflow: visible;">
					...
					<div class="tab-pane active" id="loadcontent">
						<div id="prfContent" style="width: 95%; min-height: 350px;"></div>
					</div>
					<!-- 性能曲线切换页结束 -->
					<!-- 性能数据切换页开始 -->
					<div class="tab-pane" id="dataContent" style="padding-top:10px;">
						<table class="table table-bordered table-striped table-condensed" id="conTable">
							...
							<thead>
								<c:choose>
									<c:when test="${isTbodyNotEmpty and (not empty prfData.thead) and (fn:length(prfData.thead) > 0)}">
										<tr>
	...
	<div class="row-fluid">
		<div class="box span12">
			<div class="box-header well">
				<h2 id="pTitle">事件预警</h2>
				<div class="box-icon">
					<a href="javascript:void(0);" class="btn btn-round" title="确认" data-rel="tooltip" onclick="DeviceAlert.doAlertDone('${subSystemID}','${rankInfo.storage_extent_id}','Storage');"><i class="icon-color icon-ok"></i> </a>
					<a href="javascript:void(0);" class="btn btn-round" title="删除" onclick="DeviceAlert.doAlertDel('${subSystemID}','${rankInfo.storage_extent_id}','Storage');"><i class="icon icon-color icon-trash"></i> </a>
					<a href="javascript:void(0);" class="btn btn-round" title="刷新" onclick="DeviceAlert.doFreshen('${subSystemID}','${rankInfo.storage_extent_id}','Storage');"><i class="icon icon-color icon-refresh"></i> </a>
