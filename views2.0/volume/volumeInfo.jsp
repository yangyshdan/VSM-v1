...
<%--增加查询条件--%>
	$.ajax({
		url: "${path}/servlet/volume/VolumeAction?func=VolumeSettingPrf2",
		data: {subSystemID: "${subSystemID}", level: 3, svid: "${volumeInfo.svid}" },
		type: "post",
		dataType: "html",
		...
		<div class="box span12">
			...
			<div class="box-content" id="dAlertContent">
				<table class="table table-bordered table-striped table-condensed spetable" style="table-layout:fixed;">
					...
					<tbody>
						<c:choose>
							<c:when test="${(not empty deviceLogPage) and (not empty deviceLogPage.data)}">
