...
<div class="tab-pane" id="dataContent">
	...
	<c:choose>
		<c:when test="${(not empty prfData) and (not empty prfData.tbody) and (not empty prfData.tbody.data) and fn:length(prfData.tbody.data) > 0}">
			<script>
				$("#volumeInfopageNub").getLinkStr({
					pagecount:"${prfData.tbody.totalPages}",
					curpage:"${prfData.tbody.currentPage}",
					numPerPage:"${prfData.tbody.numPerPage}",
					isShowJump:true,
					ajaxRequestPath:"${path}/servlet/volume/VolumeAction?func=VolumePrfPage&svid=${svid}&level=3&tablePage=1&subSystemID=${subSystemID}",
					divId: "dataContent"});
				$("#exportCSV").unbind();
				var exurl = "${path}/servlet/volume/VolumeAction?func=exportPrefData&svid=${svid}&level=3&subSystemID=${subSystemID}";
