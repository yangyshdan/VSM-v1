...
$(function(){
	<%--增加查询条件--%>
	$.ajax({
		url: "${path}/servlet/iogroup/IogroupAction?func=IogroupSettingPrf2",
		data: {subSystemID: "${subSystemID}", level: 3, iogroupId: "${iogroupInfo.io_group_id}" },
		type: "post",
		dataType: "html",
...
function doListRefresh(){
	loadData("${path}/servlet/iogroup/IogroupAction?func=IogroupPrfPage",{level:3,subSystemID:subSystemID,iogroupId:"${iogroupInfo.io_group_id}"},$("#perfChart"));
}
</script>
<style>
.spetable td{
...
<ul class="dashboard-list" style="margin-bottom: 10px;">
		<li style="padding-top: 0px; padding-bottom: 20px;">
			<a href="#">
				<img class="dashboard-avatar" style="border-width: 0px;" src="${path}/resource/img/project/iogroup.png" alt="StorageSystem">
			</a>
			<span style="font-size:25px;">${iogroupInfo.the_display_name}</span>
			<br>
			<strong>Status:</strong>
			<span>${iogroupInfo.maintenance}</span>
		</li>
</ul>
<div id="content">
		...
		<div class="box-content" style="width: 98%;  padding-top:10px;">
		  ...
		  <table class="table configTable" style="margin-bottom:0px;width:49%;float:left;">  
			  <tbody>
				<tr>
					<th><h4>维护状态</h4></th>
					<td class="center">${iogroupInfo.maintenance}</td>
				</tr>
			  </tbody>
		 </table>  
		</div>
	...
	<div class="row-fluid">
		<div class="box span12">
			<div class="box-header well">
				...
				<div class="box-icon">
					<!--<a href="javascript:void(0)" class="btn btn-setting btn-round" title="设置" onclick="IOGroup.settingPrf('${subSystemID}',3,'${iogroupInfo.io_group_id}')" data-rel='tooltip'><i class="icon-cog"></i></a>-->
				...
				<div id="perfChart" class="tab-content" style="overflow: visible;">
					...
					<div class="tab-pane" id="dataContent" style="padding-top:10px;">
						...
						<c:if test="${not empty prfData}">
							<script>
								$("#iogroupInfopageNub").getLinkStr({pagecount:"${prfData.tbody.totalPages}",curpage:"${prfData.tbody.currentPage}",numPerPage:"${prfData.tbody.numPerPage}",isShowJump:true,ajaxRequestPath:"${path}/servlet/iogroup/IogroupAction?func=IogroupPrfPage&iogroupId=${iogroupInfo.io_group_id}&level=3&tablePage=1&subSystemID=${subSystemID}",divId:'dataContent'});
								$("#exportCSV").unbind();
								var exurl = "${path}/servlet/iogroup/IogroupAction?func=exportPrefData&iogroupId=${iogroupInfo.io_group_id}&level=3&subSystemID=${subSystemID}";
	...
	<div class="row-fluid">
		<div class="box span12">
			<div class="box-header well">
				...
				<div class="box-icon">
					<a href="javascript:void(0);" class="btn btn-round" title="确认" data-rel="tooltip" onclick="DeviceAlert.doAlertDone('${subSystemID}','${iogroupInfo.io_group_id}','Storage');"><i class="icon-color icon-ok"></i> </a>
					<a href="javascript:void(0);" class="btn btn-round" title="删除" onclick="DeviceAlert.doAlertDel('${subSystemID}','${iogroupInfo.io_group_id}','Storage');"><i class="icon icon-color icon-trash"></i> </a>
					<a href="javascript:void(0);" class="btn btn-round" title="刷新" onclick="DeviceAlert.doFreshen('${subSystemID}','${iogroupInfo.io_group_id}','Storage');"><i class="icon icon-color icon-refresh"></i> </a>
