...
<div class="tab-pane" id="dataContent2">
	...
	<c:if test="${not empty prfData}">
		<script>
			$("#ComputerInfopageNub").getLinkStr({pagecount:"${prfData.tbody.totalPages}",
			curpage:"${prfData.tbody.currentPage}",numPerPage:"${prfData.tbody.numPerPage}",
			isShowJump:true,
			ajaxRequestPath:"${path}/servlet/x86WindowsPrf/X86WindowsServerPrfAction?func=ComputerPrfPage&computerId=${computerId}&level=3&tablePage=1",
			divId:'dataContent2'});
		</script>
	</c:if>
