<%@page import="com.huiming.sr.constants.SrContant"%>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib uri="/tags/jstl-core" prefix="c" %>

<script src="${pageContext.request.contextPath}/resource/js/project/storage.js"></script>
<style>
...
function defaultStorage(){
	...
	//$("#prfField").attr("style","color : #CC0000;");
	$("#prfField").multiselect('rebuild');
}

function resetFunc(){
	var $storageType = $("select#storageType");
	...
	$("select#prfField option").eq(0).attr({selected: true});
	$("select#time_type option").eq(0).attr({selected: true});
...
function saveInfoHgc(){
	...
	//jsonVal.push({name: "topnValue", value: topnValue});
	jsonVal.push({name: "threshold", value: threshold});
	
	$.ajax({
		url: "${pageContext.request.contextPath}/servlet/hypervisor/HypervisorAction?func=HypervisorPrf",
		data: jsonVal,
		success:function(result){
			if(result == "true"){
				bAlert("操作成功！", "提示", [{func:"doAfterSucc();", text:"确定"}]);
			}else{
				bAlert("操作失败，请稍候再试！", "提示");
			}
		}
	});
}
						...
						<tbody>
							<tr id="devtypeAndDevice">
								...
								<td id="storageTypeTd">
									...
									<c:choose>
										<c:when test="${not empty hypervisorId and hypervisorId>0 }">
											<input type="hidden" name="devId" value="${hypervisorId}" />
							...
							<tr>
								<td colspan="5" align="center" width="100%">
									...
									<a onclick="saveInfoHgc();" class="btn btn-primary">确定</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
