...
<table class="table table-bordered table-striped table-condensed" id="conTable">
	...
	<tbody>
		<c:choose>
			<c:when test="${not empty prfData and not empty prfData.tbody and not empty prfData.tbody.data}">
...
<c:choose>
	<c:when test="${(not empty prfData) and (not empty prfData.tbody) and (not empty prfData.tbody.data) and fn:length(prfData.tbody.data) > 0}">
		<script>
			$(function(){
				$("#nodeInfopageNub").getLinkStr({
					pagecount:"${prfData.tbody.totalPages}",
					curpage:"${prfData.tbody.currentPage}",
					numPerPage:"${prfData.tbody.numPerPage}",
					isShowJump:true,
					ajaxRequestPath: "${path}/servlet/node/NodeAction?func=NodePrfPage&nodeId=${nodeId}&level=3&tablePage=1&subSystemID=${subSystemID}",
					divId: "dataContent"});
				var $exportCSV = $("#exportCSV");
				$exportCSV.unbind();
				$exportCSV.attr("href", "${path}/servlet/node/NodeAction?func=ExportPrefData&nodeId=${nodeId}&level=3&tablePage=1&subSystemID=${subSystemID}");
			});
		</script>
	</c:when>
	<c:otherwise>
