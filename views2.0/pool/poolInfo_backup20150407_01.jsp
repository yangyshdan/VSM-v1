...
<%--增加查询条件--%>
	$.ajax({
			url : "${path}/servlet/pool/PoolAction?func=PoolSettingPrf2",
			data : {
	...
	function volumeFilter() {
		...
		loadData("${path}/servlet/pool/PoolAction?func=VolumeInfo", jsonVal,
				$("#volumeContent"));
	}
	...
	function doListRefresh() {
		loadData("${path}/servlet/pool/PoolAction?func=PoolPrfPage", {
			level : 3,
			subSystemID : subSystemID,
			poolId : poolId
		}, $("#perfChart"));
...
<ul class="dashboard-list" style="margin-bottom: 10px;">
	<li style="padding-top: 0px; padding-bottom: 20px;">
		<a href="#"> <img class="dashboard-avatar"
				...
				src="${path}/resource/img/project/pool.png" alt="StorageSystem">
		</a>
				...
				<table class="table configTable"
					...
					<tbody>
						...
						<tr>
							...
							<td class="center">
								<fmt:formatNumber value="${poolInfo.the_space}" pattern="0.00" />
							</td>
						</tr>
						<tr>
							<th>
							...
							<td class="center">
								<fmt:formatNumber value="${poolInfo.the_available_space}"
									pattern="0.00" />
							</td>
						</tr>
						<tr>
							...
							<td class="center">
								<fmt:formatNumber value="${poolInfo.the_unassigned_space}"
									pattern="0.00" />
							</td>
						</tr>
						<tr>
							...
							<td class="center">
								${poolInfo.the_operational_status}
							</td>
						</tr>
					</tbody>
				</table>
				<table class="table  configTable"
					...
					<tbody>
						<tr>
							...
							<td class="center">
								<fmt:formatNumber value="${poolInfo.the_consumed_space}"
									pattern="0.00" />
							</td>
						</tr>
						<tr>
							...
							<td class="center">
								<fmt:formatNumber value="${poolInfo.the_assigned_space}"
									pattern="0.00" />
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td class="center">
								${poolInfo.the_consolidated_status}
							</td>
						</tr>
						<tr>
							<th>
			...
			<div class="row-fluid">
				<div class="box span12">
					<div class="box-header well">
						...
						<div class="box-icon">
							<!--<a href="javascript:void(0)" class="btn btn-setting btn-round" title="设置" onclick="Pool.settingPrf('${subSystemID}',3,'${poolInfo.pool_id}')" data-rel='tooltip'><i class="icon-cog"></i></a>-->
							<a href="javascript:void(0);" class="btn btn-round" title="刷新"
								onclick="doListRefresh()"><i
								class="icon icon-color icon-refresh" data-rel='tooltip'></i>
							</a>
							<a href="javascript:void(0);" class="btn btn-round" title="导出"
								id="exportpoolPrfCSV"><i class="icon-download-alt"
								data-rel='tooltip'></i>
							</a>
						</div>
					</div>
					...
					<div class="box-content" style="min-height: 300px;">
						...
						<div id="perfChart" class="tab-content" style="overflow: visible;">
							...
							<div class="tab-pane active" id="loadcontent">
								<div id="prfContent" style="min-height: 350px;"></div>
							</div>
							<!-- 性能曲线切换页结束 -->
							<!-- 性能数据切换页开始 -->
							<div class="tab-pane" id="dataContent" style="padding-top: 10px;">
								...
								<c:if test="${not empty prfData}">
									<script>
										$("#poolInfopageNub").getLinkStr({
											pagecount : "${prfData.tbody.totalPages}",
											curpage : "${prfData.tbody.currentPage}",
											numPerPage : "${prfData.tbody.numPerPage}",
											isShowJump : true,
											ajaxRequestPath : "${path}/servlet/pool/PoolAction?func=PoolPrfPage&poolId=${poolId}&level=3&tablePage=1&subSystemID=${subSystemID}",
											divId : 'dataContent'
										});
										$("#exportpoolPrfCSV").unbind();
										var exurl = "${path}/servlet/pool/PoolAction?func=exportPrefData&poolId=${poolInfo.pool_id}&level=3&subSystemID=${subSystemID}";
										$("#exportpoolPrfCSV").attr("href", exurl);
									</script>
								</c:if>
								<c:if test="${empty prfData}">
								<script>
									$("#exportpoolPrfCSV").unbind();
									$("#exportpoolPrfCSV").attr("href", "javascript:void(0);");
									$("#exportpoolPrfCSV").bind("click", function() {
										bAlert("暂无可导出数据！");
									});
								</script>
								</c:if>
					...
					<div class="box-header well">
						...
						<div class="box-icon">
							<a href="javascript:void(0);" class="btn btn-round" title="确认"
								data-rel="tooltip"
								onclick="DeviceAlert.doAlertDone('${subSystemID}','${poolInfo.pool_id}','Storage');"><i
								class="icon-color icon-ok"></i> </a>
							<a href="javascript:void(0);" class="btn btn-round" title="删除"
								onclick="DeviceAlert.doAlertDel('${subSystemID}','${poolInfo.pool_id}','Storage');"><i
								class="icon icon-color icon-trash"></i> </a>
							<a href="javascript:void(0);" class="btn btn-round" title="刷新"
								onclick="DeviceAlert.doFreshen('${subSystemID}','${poolInfo.pool_id}','Storage');"><i
								...
								<tr>
									<th style="width: 20px;">
										<label class="checkbox inline">
											<input type="checkbox"
												onclick="DeviceAlert.doAlertCheck(this.checked);">
										</label>
									</th>
									...
									<c:when test="${not empty deviceLogPage.data}">
										<c:forEach var="item" items="${deviceLogPage.data}"
											...
											<tr style="cursor: pointer;"
												ondblclick="DeviceAlert.doDetailInfo('${item.fruleid}','${item.ftopid}','Storage')">
												<td>
													<label class="checkbox inline">
														<input type="checkbox"
												...
												<td>
													<a
														href="javascript:DeviceAlert.doDetailInfo('${item.fruleid}','${item.ftopid}','Storage')"
			...
			.getLinkStr(
					{
						...
						isShowJump : true,
						ajaxRequestPath : "${path}/servlet/alert/DeviceAlertAction?func=AjaxPage&resourceId=${poolInfo.pool_id}&topId=${subSystemID}&resourceType=Storage",
						divId : 'dAlertContent'
					});
</script>
						</c:if>
			...
			<div class="row-fluid">
				<div class="box span12">
					<div class="box-header well" data-original-title>
						...
						<div class="box-icon">
							<a href="javascript:void(0)" class="btn btn-round" title="刷新"
								onclick="volumeFilter();" data-rel='tooltip'><i
								class="icon icon-color icon-refresh"></i> </a>
							<a href="javascript:void(0)" class="btn btn-round" title="导出"
								id="exportCSV"><i class="icon-download-alt"></i> </a>
							<a href="javascript:void(0)" class="btn btn-minimize btn-round"><i
								class="icon-chevron-up" data-rel='tooltip'></i> </a>
							<script>
					...
					<div class="box-content">
						<div id="perfChart" class="tab-content" style="overflow: visible;">
							<div class="tab-pane active" id="volumeContent"
								...
								<div id="volumeContent" style="min-height: 300px;">
									<table
										class="table table-bordered table-striped table-condensed"
										id="conTable">
										<thead>
										...
										<tbody>
											<c:choose>
												<c:when test="${not empty volumePage.data}">
													<c:forEach var="item" items="${volumePage.data}"
														varStatus="status">
														<tr>
															<td>
															...
															<td>
																<a title="${item.sub_name}"
																	href="${path}/servlet/storage/StorageAction?func=StorageInfo&subSystemID=${item.subsystem_id}">${item.sub_name
																	}</a>
															</td>
															<td>
															...
															<td>
																<fmt:formatNumber value="${item.the_capacity}"
																	pattern="0.00" />
															</td>
															<td>
																<fmt:formatNumber value="${item.the_used_space}"
																	pattern="0.00" />
															</td>
															<td>
																${item.the_redundancy}
			...
			.getLinkStr(
					{
						...
						isShowJump : true,
						ajaxRequestPath : "${path}/servlet/pool/PoolAction?func=VolumeInfo&poolId=${poolId}&isFreshen=1",
						divId : 'volumeContent'
					});
</script>
									</c:if>
