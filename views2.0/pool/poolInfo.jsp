...
<%--$(function(){
	...
	$.ajax({
		...
		data: {subSystemID: "${subSystemID}", level: 3, poolId: "${poolInfo.pool_id}" },
		type: "post",
		dataType: "html",
		...
		<div class="box span12">
			<div class="box-header well">
				...
				<div class="box-icon">
					...
					<a href="javascript:void(0);" class="btn btn-round" title="导出" id="exportpoolPrfCSV"><i class="icon-download-alt" data-rel='tooltip'></i></a>
				</div>
			</div>
				...
				<div id="perfChart" class="tab-content" style="overflow:visible;">
					...
					<div class="tab-pane" id="dataContent" style="padding-top:10px;">
						...
						<c:if test="${not empty prfData}">
							<script>
							$("#poolInfopageNub").getLinkStr({pagecount:"${prfData.tbody.totalPages}",curpage:"${prfData.tbody.currentPage}",numPerPage:"${prfData.tbody.numPerPage}",isShowJump:true,ajaxRequestPath:"${path}/servlet/pool/PoolAction?func=PoolPrfPage&poolId=${poolId}&level=3&tablePage=1&subSystemID=${subSystemID}",divId:'dataContent'});
						...
						<c:if test="${empty prfData}">
							<script>
								...
								$("#exportpoolPrfCSV").bind("click",function(){bAlert("暂无可导出数据！")});
							</script>
						</c:if>
					</div>
					<!-- 性能数据切换页结束 -->
