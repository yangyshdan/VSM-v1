...
$(function(){
	<%--增加查询条件--%>
	$.ajax({
		url: "${path}/servlet/port/PortAction?func=PortSettingPrf2",
		data: {subSystemID: "${subSystemID}", level: 3, portId: "${portInfo.port_id}" },
		type: "post",
		dataType: "html",
...
function doListRefresh(){
	loadData("${path}/servlet/port/PortAction?func=PortPrfPage",{level:3,subSystemID:subSystemID,portId:"${portInfo.port_id}"},$("#perfChart"));
}
</script>
<style>
.spetable td{
...
<ul class="dashboard-list" style="margin-bottom: 10px;">
	<li style="padding-top: 0px; padding-bottom: 20px;">
		<a href="#">
			<img class="dashboard-avatar" style="border-width: 0px;" src="${path}/resource/img/project/storageport.png" alt="StorageSystem">
		</a>
	...
	<div class="tab-pane active" id="detailTab">
		...
		<table class="table  configTable" style="margin-bottom:0px;width:49%;float:left;">  
			  <tbody>
				<tr>
					<th><h4>存储系统</h4></th>
					<td class="center">${portInfo.sub_name}</td>                                       
				</tr>
				<tr>
					<th><h4>端口类型</h4></th>
					<td class="center">${portInfo.the_type}</td>                                       
				</tr>
				<tr>
					<th><h4>硬件状态</h4></th>
					<td class="center">${portInfo.the_consolidated_status}</td>                                       
				</tr>
			  </tbody>
		 </table>  
		<!-- 存储系统详细信息表单结束 -->
	...
	<div class="row-fluid">
		<div class="box span12">
			<div class="box-header well">
				<h2 id='pTitle'>性能</h2>
				<div class="box-icon">
					<%--<a href="javascript:void(0)" class="btn btn-setting btn-round" title="设置" onclick="Port.settingPrf('${subSystemID}',3,'${portInfo.port_id}')" data-rel='tooltip'><i class="icon-cog"></i></a>--%>
				...
				<div id="perfChart" class="tab-content" style="overflow: visible;">
					...
					<div class="tab-pane" id="dataContent" style="padding-top:10px;">
						...
						<c:if test="${isTbodyNotEmpty}">
							<script>
								$("#portInfopageNub").getLinkStr({pagecount:"${prfData.tbody.totalPages}",curpage:"${prfData.tbody.currentPage}",numPerPage:"${prfData.tbody.numPerPage}",isShowJump:true,ajaxRequestPath:"${path}/servlet/port/PortAction?func=PortPrfPage&portId=${portInfo.port_id}&level=3&tablePage=1&subSystemID=${subSystemID}",divId:'dataContent'});
								var $csv = $("#exportCSV");
								$csv.unbind();
	...
	<div class="row-fluid">
		<div class="box span12">
			<div class="box-header well">
				...
				<div class="box-icon">
					<a href="javascript:void(0);" class="btn btn-round" title="确认" data-rel="tooltip" onclick="DeviceAlert.doAlertDone('${subSystemID}','${portInfo.port_id}','Storage');"><i class="icon-color icon-ok"></i> </a>
					<a href="javascript:void(0);" class="btn btn-round" title="删除" onclick="DeviceAlert.doAlertDel('${subSystemID}','${portInfo.port_id}','Storage');"><i class="icon icon-color icon-trash"></i> </a>
					<a href="javascript:void(0);" class="btn btn-round" title="刷新" onclick="DeviceAlert.doFreshen('${subSystemID}','${portInfo.port_id}','Storage');"><i class="icon icon-color icon-refresh"></i> </a>
			...
			<div class="box-content" id="dAlertContent">
				...
				<c:if test="${not empty deviceLogPage.data}">
					<script>
					$("#alertListNub").getLinkStr({pagecount:"${deviceLogPage.totalPages}",curpage:"${deviceLogPage.currentPage}",numPerPage:"${deviceLogPage.numPerPage}",isShowJump:true,ajaxRequestPath:"${path}/servlet/alert/DeviceAlertAction?func=AjaxPage&resourceId=${portInfo.port_id}&topId=${subSystemID}&resourceType=Storage",divId:'dAlertContent'});
					</script>
				</c:if>
			</div>
		</div>
