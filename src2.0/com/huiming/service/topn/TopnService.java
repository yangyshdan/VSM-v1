...
public class TopnService extends BaseService{
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<DataRow> getStorageByType(String storageType) {
		String sql="select subsystem_id as ele_id,name as ele_name from t_res_storagesubsystem where storage_type = ?";
		return getJdbcTemplate(WebConstants.DB_DEFAULT).query(sql, new Object[]{storageType});
	}
	...
	public JSONArray getPrfJSON(DataRow row){
		...
		for (String str : kpi) {
			...
			StringBuffer sb = new StringBuffer(sql);
			String dbType = tnpRow.getString("fdbtype");
	...
	public JSONArray getPrfJSON3(DataRow row){
		...
		for (String str : kpi) {
			...
			//获取指定性能指标的详细信息
			DataRow tnpRow = getTnptableInfo(str);
			String sql = "select p.prf_timestamp,AVG(p." + str + ") as " + str + ",p.ele_name from ";
			StringBuffer sb = new StringBuffer(sql);
			String dbtype = tnpRow.getString("fdbtype");
			String tableName = row.getString("fprfview");
			String timeSize = row.getString("time_size");
			...
			if (dbtype.equals(SrContant.DBTYPE_SR)) {
				...
				String strBeforeOrder = sb.substring(0, orderIndex).replaceFirst("p.prf_timestamp", "min(p.prf_timestamp)");
			...
			for (int i = 0; i < rows.size(); i++) {
				...
				objs.put("time", rows.get(i).getString("prf_timestamp").toString());
				objs.put("y", Double.parseDouble(decFmt.format(rows.get(i).getDouble(str.toLowerCase()))));
				jsonAry.add(objs);
			}
			obj.put("ftitle", tnpRow.getString("ftitle"));
			obj.put("names", names);
