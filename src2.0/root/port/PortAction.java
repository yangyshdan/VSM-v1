...
public class PortAction extends SecurityAction{
	...
	public ActionResult doPortSettingPrf(){
		...
		if(devId!=null && devId>0){
			DataRow config = baseService.getPrfFieldInfo(null,3, "Port", row.getString("type"),subsystemId,devId,getLoginUserId());
			if(config==null){
			...
			this.setAttribute("historyConfig", baseService.getPrfFieldInfo(null,2, "Port", row.getString("type"),subsystemId,null,getLoginUserId()));
	...
	public void doPortPrfField(){
		...
		DataRow row = baseService.getStorageType(subsystemId);
		DataRow dataRow = baseService.getPrfFieldInfo(null, level, "Port", row.getString("type"),
				subsystemId, devId,getLoginUserId());
		...
		if(dataRow != null && dataRow.size() > 0){
			List<DataRow> devs = baseService.getDeviceInfo(dataRow.getString("fdevice"), "port_id",
					"the_display_name", "v_res_port");
	...
	public void doExportPrefData(){
		...
		Integer subsystemId = getIntParameter("subSystemID");
		DataRow row = baseService.getStorageType(subsystemId);
		DataRow dataRow = baseService.getPrfFieldInfo(null,level,"Port", row.getString("type"),subsystemId,devId,getLoginUserId());
