...
public class StoragenodeAction extends SecurityAction{
	...
	public ActionResult doAjaxNodePage(){
		...
		String spName = getStrParameter("spName").replaceAll("&amp;nbsp;", " ");
		int curPage = getIntParameter("curPage",1);
		int numPerPage = getIntParameter("numPerPage",SrContant.SR_NumPerPage);
		DBPage nodePage = nodeService.getNodePage(spName, subsystemId, curPage, numPerPage);
		setAttribute("spName", spName);
		setAttribute("nodePage", nodePage);
		setAttribute("subSystemID", subsystemId);
		setAttribute("storageType", storageType);
		return new ActionResult("/WEB-INF/views/sr/sp/nodePage.jsp");
	}
	 ...
	 * @return
	 */
	public ActionResult doLoadStoragenodeInfo(){
		Integer subsystemId = getIntParameter("subsystemId");
		Integer spId = getIntParameter("spId");
		String storageType = getStrParameter("storageType");
		//控制器详细信息
		DataRow storagenode = nodeService.getStoragenodeIfo(subsystemId, spId);
		int curPage = getIntParameter("curPage",1);
		int numPerPage = getIntParameter("numPerPage",SrContant.SR_NumPerPage);
		//卷信息列表
		DBPage volumePage = nodeService.getspVolumeInfo(spId, curPage, numPerPage);
		
		setAttribute("storagenode", storagenode);
		setAttribute("subsystemId", subsystemId);
		setAttribute("spId", spId);
		setAttribute("storageType", storageType);
		setAttribute("volumePage", volumePage);
		return new ActionResult("/WEB-INF/views/sr/sp/spInfo.jsp");
	}
	...
	public ActionResult doLoadPerfInfo(){
		...
		if (StringHelper.isNotEmpty(tablePage) && StringHelper.isNotBlank(tablePage)) {
			return new ActionResult("/WEB-INF/views/sr/sp/spPerfPage.jsp");
		}
		return new ActionResult("/WEB-INF/views/sr/sp/spWriteLine.jsp");
	}
	...
	public void getPerfInfoData(){
		Integer subsystemId = getIntParameter("subsystemId");
		Integer devId = getIntParameter("spId");
		String storageType = getStrParameter("storageType");
		String tablePage = getStrParameter("tablePage");
		...
		DataRow dataRow = baseprfService.getPrfFieldInfo(null,level,SrContant.SUBDEVTYPE_NODE,storageType,subsystemId,devId,getLoginUserId());
		DataRow thead = new DataRow();
		DBPage tbody = null;
		//给默认性能信息
		if (dataRow == null || dataRow.size() == 0) {
			dataRow = baseprfService.getDefaultRow(SrTblColConstant.TBL_RES_STORAGENODE, devId, storageType, SrContant.SUBDEVTYPE_NODE, SrTblColConstant.RSN_SP_ID, SrTblColConstant.RSN_SP_NAME);
		}
		
		if(dataRow != null && dataRow.size() > 0){
			List<DataRow> devs = baseprfService.getDeviceInfoList(dataRow.getString("fdevice"), SrTblColConstant.RSN_SP_ID, SrTblColConstant.RSN_SP_NAME, SrTblColConstant.TBL_RES_STORAGENODE);
	...
	public ActionResult doPerfChartSetting(){
		...
		//获取性能指标
		List<DataRow> kpiList = baseprfService.getView(storageType, SrContant.SUBDEVTYPE_NODE);
		List<DataRow> devList = baseprfService.getDeviceList(subsystemId, SrTblColConstant.TBL_RES_STORAGENODE, SrTblColConstant.REF_SUBSYSTEM_ID, SrTblColConstant.RSN_SP_ID, SrTblColConstant.RSN_SP_NAME);
		...
		if (devId != null && devId > 0) {
			//获取历史配置性能信息
			DataRow config = baseprfService.getPrfFieldInfo(null, 3, SrContant.SUBDEVTYPE_NODE, storageType, subsystemId, devId, getLoginUserId());
			if (config == null) {
			...
			setAttribute("historyConfig",baseprfService.getPrfFieldInfo(null, 2, SrContant.SUBDEVTYPE_NODE, storageType, subsystemId, null, getLoginUserId()));
			setAttribute("level", 2);
		}
		setAttribute("url", "servlet/sr/storagenode/StoragenodeAction?func=PerfSetting");
	...
	public void doPerfSetting(){
		...
		row.set("level", level);
		row.set("fname", SrContant.SUBDEVTYPE_NODE);
...
//		row.set("fisshow", showname);
		...
		try {
			baseprfService.updatePrfField(row, SrContant.SUBDEVTYPE_NODE, storageType, devId, subsystemId, level);
	 ...
	 * @return
	 */
	public ActionResult doSPvolumePage(){
		int curPage = getIntParameter("curPage",1);
		Integer subsystemId = getIntParameter("subsystemId");
		String storageType = getStrParameter("storageType");
		int numPerPage = getIntParameter("numPerPage",SrContant.SR_NumPerPage);
		...
		setAttribute("spId", spId);
		setAttribute("storageType", storageType);
		return new ActionResult("/WEB-INF/views/sr/sp/ajaxVolume.jsp");
	}
	...
	public void doExpertVolumeConfigData(){
		Integer spId = getIntParameter("spId");
		List<DataRow> rows = nodeService.getspVolumeInfo(spId);
	...
	public void doExportConfigData(){
		Integer subsystemId = getIntParameter("subSystemID");
		List<DataRow> rows = nodeService.getNodesInfo(null,subsystemId);
		if (rows != null && rows.size() > 0) {
			String[] title = new String[]{"控制器名称","系统","序列号","编号","EMC汇编修订","可设计名称","更新时间"};
			String[] keys = new String[]{"sp_name","model","emc_serial_number","emc_part_number","emc_assembly_revision","programmable_name","update_timestamp"};
			getResponse().setCharacterEncoding("GBK");
			CSVHelper.createCSVToPrintWriter(getResponse(), "SP-ConfigrationData", rows, title, keys);
		}
	}
