...
<div class="tab-pane" id="dataContent">
	...
	<c:if test="${not empty prfData}">
		<script>
			$("#volumeInfopageNub").getLinkStr({pagecount:"${prfData.tbody.totalPages}",curpage:"${prfData.tbody.currentPage}",numPerPage:"${prfData.tbody.numPerPage}",isShowJump:true,ajaxRequestPath:"${path}/servlet/volume/VolumeAction?func=VolumePrfPage&svid=${svid}&level=3&tablePage=1&subSystemID=${subSystemID}",divId:'dataContent'});
		</script>
	</c:if>
