...
<div class="tab-pane" id="dataContent2" style="padding-top:10px;">
	...
	<div id="appInfopageNub" class="pagination pagination-centered"></div>
	<c:if test="${not empty prfData}">
		<script>
		$("#appInfopageNub").getLinkStr({pagecount:"${prfData.tbody.totalPages}",curpage:"${prfData.tbody.currentPage}",numPerPage:"${prfData.tbody.numPerPage}",isShowJump:true,ajaxRequestPath:"${path}/servlet/apps/AppsAction?func=AppsPrfPage&fappId=${fappId}&level=3&tablePage=1",divId:'dataContent2'});
		</script>
	</c:if>
	<c:if test="${empty prfData}">
		<script>
			...
			$("#exportCSV").bind("click",function(){bAlert("暂无可导出数据！+00000")});
		</script>
	</c:if>
</div>
