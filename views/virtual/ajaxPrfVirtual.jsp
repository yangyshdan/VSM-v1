...
<div class="tab-pane" id="dataContent2">
	...
	<c:if test="${not empty prfData.tbody.data}">
		<script>
		$("#virtualInfopageNub").getLinkStr({pagecount:"${prfData.tbody.totalPages}",curpage:"${prfData.tbody.currentPage}",numPerPage:"${prfData.tbody.numPerPage}",isShowJump:true,ajaxRequestPath:"${path}/servlet/virtual/VirtualAction?func=VirtualPrfPage&vmId=${vmId}&level=3&tablePage=1",divId:'dataContent2'});
		</script>
	</c:if>
	<c:if test="${not empty prfData.tbody}">
		<script>
			$("#exportCSV").unbind();
