...
<div class="tab-pane" id="dataContent">
	...
	<c:if test="${not empty prfData}">
		<script>
		$("#portInfopageNub").getLinkStr({pagecount:"${prfData.tbody.totalPages}",curpage:"${prfData.tbody.currentPage}",numPerPage:"${prfData.tbody.numPerPage}",isShowJump:true,ajaxRequestPath:"${path}/servlet/port/PortAction?func=PortPrfPage&portId=${portId}&level=3&tablePage=1&subSystemID=${subSystemID}",divId:'dataContent'});
		</script>
	</c:if>
